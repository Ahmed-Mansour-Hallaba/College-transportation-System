...
$drslt=mysqli_query($dcn,"select d.id,u.fullname,d.mobile,b.busnumber,b.color,b.capacity,d.state from drivers d join users u on(d.id=u.id) join buses b on(d.bus_id=b.id) where state='accepted' and d.id='$login'");
$month=date('m');
$year=date('Y');
...
<script>
    function sub() {
        ...
        var y=d.getFullYear();
        m++;
        window.location.href = "salary.php?m="+m+"&y="+y;
